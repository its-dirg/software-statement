...
class Client(oic.Client):
    def __init__(self, conf, software_statement, client_id=None, ca_certs=None,
    ...
    def callback(self, response, session):
        ...
        if self.behaviour["response_type"] == "code":
            ...
            try:
                ...
                if self.registration_response:
                    authn_method = self.registration_response["token_endpoint_auth_method"]
                else:
                    authn_method = self.config.ME['token_endpoint_auth_method']
...
class OIDCClients(object):
    def __init__(self, config, software_statement, keyjar=None):
        """
    ...
    def create_client(self, userid="", **kwargs):
        ...
        client = self.client_cls(self.config, self.software_statement,
                                 client_authn_method=CLIENT_AUTHN_METHOD,
                                 behaviour=kwargs["behaviour"], verify_ssl=self.config.VERIFY_SSL,
                                 keyjar=self.keyjar, **args)
    ...
    def dynamic_client(self, userid):
        client = self.client_cls(self.config, self.software_statement,
                                 client_authn_method=CLIENT_AUTHN_METHOD,
                                 verify_ssl=self.config.VERIFY_SSL, keyjar=self.keyjar)
        ...
        if issuer in self.client:
            ...
            if self.config.DO_CLIENT_REGISTRATION:
                ...
                client.client_id = client.software_statement
